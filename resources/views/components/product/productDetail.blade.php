...
<div  iv class="container d-flex justify-content-center flex-wrap">
  ...
  <div class="card d-flex flex-row">
    ...
    <div class="card-body d-flex flex-column justify-content-center">
      ...
      <p class="card-text">Category: {{$product->category_name}}</p>
      <p class="card-text" id="productDesc">{{$product->description}}</p>
      <a href="#" class="btn btn-success mb-4">Edit Product</a>
...
@else
    ...
    <div class="card-body d-flex flex-column justify-content-center">
        ...
        <p class="card-text">Category: {{$product->category_name}}</p>
        <p class="card-text" id="productDesc">{{$product->description}}</p>
        <a href="{{ url('products/profile/?q='.Crypt::encrypt($product->id)); }}" class="btn btn-primary disabled" aria-disabled="true">Purchase</a>
    </div>
    </div>
    @endif    
 @endguest
